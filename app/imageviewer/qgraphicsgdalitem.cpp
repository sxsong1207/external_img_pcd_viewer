...
#include <omp.h>
#endif
QGraphicsGDALItem::QGraphicsGDALItem(QGraphicsItem *parent): QGraphicsItem(parent),pdata(NULL) {
    number_bands=0;
    geoRefInited=false;
...
//            qDebug()<<"GeoProj"<<pdata->GetProjectionRef();
            poTiff2WGS84=NULL;
            poWGS84ToTiff=NULL;
            try {
...
void QGraphicsGDALItem::close_img()
{
    if(pdata)
    {
        GDALClose(pdata);
